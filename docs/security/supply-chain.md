...
**Externally-verifiable signals**: each public OpenSecOps-Org repository surfaces two third-party signals in its README, both readable without OpenSecOps cooperation:

- **OpenSSF Scorecard** — automated supply-chain posture analysis covering ~18 dimensions (branch protection, signed releases, pinned dependencies, dangerous workflow patterns, etc.), published at https://scorecard.dev/. The score is the single most-frequently-cited supply-chain signal in 2026 FOSS-intake conversations.
...
| Is there an OpenSSF Scorecard score?                       | Yes — published continuously at scorecard.dev for every public OpenSecOps-Org repository, surfaced as a badge in each repo's README (§5.5). |
