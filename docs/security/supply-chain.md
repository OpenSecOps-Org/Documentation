...
The supply-chain posture described in §5–§9 applies to every OpenSecOps component. All have been **converted** to the framework: every release is built and signed via the same `./publish` toolchain, with Sigstore keyless OIDC signatures attached as GitHub Release assets on the published `OpenSecOps-Org/<component>` repository. Conversion takes two shapes depending on whether the component ships Python dependencies:

- **Python-bearing components** carry hand-edited `requirements.in` files declaring direct dependencies for each Lambda function or entry point, machine-generated hash-verified `requirements.txt` lock files, a per-component `.security-config.toml` and generated `SECURITY.md` rendered from the project-wide template, and per-function CycloneDX SBOMs (`requirements.cdx.json`) plus PyPI publisher-provenance baselines (`requirements.provenance.json`) committed alongside each `requirements.in`.
- **Libraryless components** — those that ship only CloudFormation templates, JSON policy documents, or rendered documentation — emit a deterministic `git archive HEAD` source tarball plus a SLSA Build L1 in-toto provenance document at release time in place of the per-function SBOM and evidence set. Each release artefact is Sigstore-signed identically to the Python-bearing path.

The converted set: **Documentation**, **Foundation-AWS-Core-SSO-Configuration**, **Foundation-CloudWatch2S3**, **Foundation-control-tower-log-aggregator**, **Foundation-default-vpc-remover**, **Foundation-enable-ebs-encryption-by-default**, **Foundation-iam-password-policy**, **Foundation-infra-immutable-tagger**, **Foundation-instance-port-report**, **Foundation-limit-log-group-retention**, **Foundation-new-account-created-sns-topic**, **Foundation-permission-boundary-policies**, **Foundation-resource-control-policies**, **Foundation-security-services-setup**, **Foundation-service-control-policies**, **Installer**, **SOAR**, **SOAR-all-alarms-to-sec-hub**, **SOAR-detect-log-buckets**, **SOAR-detect-stack-drift**, **SOAR-SAM-Automating-Forensic-Disk-Collection**, **SOAR-sec-hub-configuration**, **SOAR-sec-hub-role**, and **SOAR-soc-incident-when-s3-tag-applied**. Every component is held to the same uniform posture, with no per-component variation and no quiet exemptions.
...
| Is there an OpenSSF Best Practices Badge?                  | Yes — Passing level, project entry at [bestpractices.dev/projects/12827](https://www.bestpractices.dev/projects/12827). Self-attestation against the OpenSSF Best Practices criteria covering FLOSS hygiene, vulnerability disclosure, signed releases, hash-pinned dependencies, and continuous detection. Displayed as a badge in each component's `README.md`. |
